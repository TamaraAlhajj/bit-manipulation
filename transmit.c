...

{
	...
	sum = *num & mask;
    	...
    	// if the number of bits is odd then set the parity bit P1
	if(numberOfOnes(sum) % 2 != 0) setShortBit(1, num);
	...
	if(numberOfOnes(sum) % 2 != 0) setShortBit(2, num);
	...
	// get the bits covered by the mask for P4
    	...
    	// count the number of bits and set the parity of P4 
	if(numberOfOnes(sum) % 2 != 0) setShortBit(4, num);

    	// set parity bit p8
	mask = P8_MASK;
	
	// get the bits covered by the mask for P8
   	 sum = *num & mask;

    	// count the number of bits and set the parity of P8 
	if(numberOfOnes(sum) % 2 != 0) setShortBit(8, num);
